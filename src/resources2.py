...
def _get_filename(base, name):
    result = None
    for folder in ["campaigns/common", "extra", "core"]:
        result = os.path.join("data", folder, base, name)
...
class FontLoader:
    def __init__(self):
        self.fonts = {}
        self.files = {
            "sans": {False: "assets/fonts/vera/Vera.ttf", True: "assets/fonts/vera/VeraBd.ttf"},
            "serif": {False: "assets/fonts/vera/VeraSe.ttf", True: "assets/fonts/vera/VeraSeBd.ttf"},
            "mono": {False: "assets/fonts/vera/VeraMono.ttf", True: "assets/fonts/vera/VeraMoBd.ttf"}
        }
    ...
    def __call__(self, family="sans", size=16, bold=True):
        ...
        if key not in self.fonts:
            ...
            font_file = _get_filename("fonts", filename)
            if font_file is None:
                raise Exception('Font file "%s" not found' % filename)
            self.fonts[key] = pygame.font.Font(font_file, size)
...
def set_campaign(c):
    global campaign, font, map, image, texture, scenario, class_, ability, weapon, sprite_config
    ...
    sprite_config = SpriteConfigLoader()
