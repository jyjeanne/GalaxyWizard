...
def makeCube(z, cornerHeights, texture, cornerColors, waterHeight, waterColor, minHeight):
    ...
    glTexCoord2f(0.0, llz)
    glVertex3f(-0.5, -0.5, llz)
    glColor4f(cc[1][3][0], cc[1][3][1], cc[1][3][2], cc[1][3][3])
    glTexCoord2f(1.0, ulz)
    glVertex3f(-0.5, 0.5, ulz)
    ...
    glTexCoord2f(0.0, ulz)
    glVertex3f(-0.5, 0.5, ulz)
    glColor4f(cc[2][3][0], cc[2][3][1], cc[2][3][2], cc[2][3][3])
    glTexCoord2f(1.0, urz)
    glVertex3f(0.5, 0.5, urz)
    ...
    glTexCoord2f(0.0, lrz)
    glVertex3f(0.5, -0.5, lrz)
    ...
    # four corners
    (mr, mg, mb, ma) = (
        (cc[0][0][0] + cc[0][1][0] + cc[0][2][0] + cc[0][3][0]) / 4,
...
def drawAt(xxx_todo_changeme2, texture, image):
    (posx, posy) = xxx_todo_changeme2
...
def drawBar(leftColor, rightColor, barWidth, barHeight, xxx_todo_changeme3):
    (posx, posy) = xxx_todo_changeme3
    (width, height) = MainWindow.get().size()
...
def createBorder(width, height, image=None):
    ...
    pygame.draw.rect(
        bordered,
        (255, 255, 255, 210),
        pygame.Rect((1, 1), (bordered.get_width() - 2, bordered.get_height() - 2)),
        borderWidth,
    )
    pygame.draw.rect(
        bordered,
...
def drawText(
    font, color, text, xxx_todo_changeme4, border, backgroundColor, centerx=False, centery=False
):
    (posx, posy) = xxx_todo_changeme4
    (texture, image, imageSize) = renderTextToTexture(font, color, text, border, backgroundColor)
    if centerx:
        posx -= imageSize[0] / 2
    if centery:
        posy -= imageSize[1] / 2
...
def makeTexture(textureSurface, backgroundColor=None, textureID=-1):
    ...
    if width not in powersOf2:
        for i in range(0, len(powersOf2)):
...
def drawFloatingText(texture, aspectRatio, height, alpha):
    # Imported lazily: ScenarioGUI -> Cursor -> Sprite -> resources -> GLUtil
    # would otherwise form a circular import at module load time.
...
def makeUnit(
    ...
    from gui import ScenarioGUI  # lazy import to avoid circular import
