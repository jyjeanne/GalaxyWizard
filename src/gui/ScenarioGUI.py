...
class ScenarioGUI(MainWindow.MainWindowDelegate):
    def __init__(self, client, scenario, faction):
        ...
        self._centeredTextDisplayer.setPosn(
            (MainWindow.get().size()[0] / 2, MainWindow.get().size()[1] / 2)
        )
                ...
                for i in range(0, m.width):
                    ...
                    if "listID" in sq.guiData:
                        try:
                            ...
                            logging.warning(
                                f"Failed to delete display list for square ({i},{j}): {e}"
                            )
                        except Exception as e:
                            import logging

                            logging.error(
                                f"Unexpected error deleting display list for square ({i},{j}): {e}"
                            )
                    if "topListID" in sq.guiData:
                        try:
                            ...
                            logging.warning(
                                f"Failed to delete top display list for square ({i},{j}): {e}"
                            )
                        except Exception as e:
                            import logging

                            logging.error(
                                f"Unexpected error deleting top display list for square ({i},{j}): {e}"
                            )
    ...
    def showEffect(self, targetID, effect, delay):
        ...
        if isinstance(effect, Effect.MissResult):
            ...
            ud.addAnimation(dd)
        elif isinstance(effect, Effect.DamageResult) or isinstance(effect, Effect.DamageSPResult):
            dd = Sprite.DamageDisplayer(effect.damage, Sprite.NEGATIVE, delay)
            ud.addAnimation(dd)
        elif isinstance(effect, Effect.HealResult):
            dd = Sprite.DamageDisplayer(effect.damage, Sprite.BENEFICIAL, delay)
            ud.addAnimation(dd)
    ...
    def setTopText(self, text):
        ...
        self._topTextDisplayerClearer = reactor.callLater(2, self.clearTopText)
    ...
    def _updateUnitSlide(self, timeElapsed):
        ...
        if self.unitMoveZDiff > 0:
            if self.lastUnitMove < squareMoveTime * zSpeedup:
                dz = timeElapsed * (1.0 / squareMoveTime) * zSpeedup * self.unitMoveZDiff
                if sq.z - ud.z < dz:
                    dz = sq.z - ud.z
        elif self.unitMoveZDiff < 0:
            if self.lastUnitMove > squareMoveTime * (zSpeedup - 1) / zSpeedup:
                dz = timeElapsed * (1.0 / squareMoveTime) * zSpeedup * self.unitMoveZDiff
                if ud.z - sq.z < dz:
                    dz = sq.z - ud.z
        if abs(dz) < 0.01:
    ...
    def drawBG(self):
        ...
        for s in self.sortedMapSquares:
            ...
            if (s.x, s.y) in self.highlights.keys():
                ...
                a = a * self.highlightAlpha() * self.cursorHighlightAlpha
                glColor4f(r, g, b, a)
    ...
    def update(self, timeElapsed):
        ...
        if self.fsm.state == "abilityTarget":
            self.showAbilityRange(self.unit, self.fsm.ability)
            self.showAbilityAOE(self.unit, self.fsm.ability, (self.cursor.x, self.cursor.y))
...
class ScenarioGUIFSM(FSM.FSM):
    ...
    def accept_abilityTarget(self):
        df = self.gui.client.remote(
            "unitAct", self.ability.abilityID, self.gui.cursor.x, self.gui.cursor.y
        )
    ...
    def accept_moveTarget(self):
        df = self.gui.client.remote("unitMove", self.gui.cursor.x, self.gui.cursor.y)
    ...
    def accept_facing(self):
        df = self.gui.client.remote("unitFacing", self.gui.unit.facing())
