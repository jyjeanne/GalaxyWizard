...
class MapEditorGUI(MainWindow.MainWindowDelegate):
    def __init__(self, m):
        ...
        self.lightEnv.addLight(Light.White(brightness=0.5, position=[20, 15, 100]))
        self.lightEnv.setFog(color=[0.4, 0.4, 0.5], start=10.0, end=30.0)
        self.setLighting()
        ...
        self._addTagDialog = MapEditorSprite.AddTagDialog(
            (50, 50), MainWindow.get().size()[0] - 130
        )
        self._editTagDialog = MapEditorSprite.EditTagDialog(
            (50, 50), MainWindow.get().size()[0] - 130
        )
        self._saveDialog = MapEditorSprite.SaveDialog((50, 50), MainWindow.get().size()[0] - 130)
        self._tileInfoDisplayer = MapEditorSprite.TileInfoDisplayer(
            (50, 50), MainWindow.get().size()[0] - 130
        )
                ...
                for i in range(0, m.width):
                    ...
                    if "listID" in sq.guiData:
                        try:
                            ...
                            logging.warning(
                                f"Failed to delete display list for map square ({i},{j}): {e}"
                            )
                        except Exception as e:
                            import logging

                            logging.error(
                                f"Unexpected error deleting display list for map square ({i},{j}): {e}"
                            )
                    if "topListID" in sq.guiData:
                        try:
                            ...
                            logging.warning(
                                f"Failed to delete top display list for map square ({i},{j}): {e}"
                            )
                        except Exception as e:
                            import logging

                            logging.error(
                                f"Unexpected error deleting top display list for map square ({i},{j}): {e}"
                            )
    ...
    def handleEvent(self, event):
        ...
        if (
            self.activeDelegate != self.delegates["addTag"]
            and self.activeDelegate != self.delegates["save"]
        ):
