...
class SecondHeightDisplayer(TileDataDisplayer):
    ...
    def getText(self):
        ...
        cornerHeights = self.cursor.mapSquare().cornerHeights
        return "%2d   %2d   %2d" % (
            cornerHeights[self._camera.getCorner(2)] + height,
            height,
            cornerHeights[self._camera.getCorner(1)] + height,
        )
...
class TopMenu(Sprite.TextMenu):
    ...
    def __init__(self, posn):
        ...
        self.setOptions(
            ["Tile Info", "Set Terrain Tag", "Edit Terrain Tag", "New Terrain Tag", "Save"]
        )
...
class Dialog(Sprite.TextDisplayerBox):
    def __init__(self, title, inputBoxes, inputTitles, posn, width, height=None):
        self.title = title
    ...
    def handleEvent(self, event):
        if event.type == pygame.KEYDOWN and event.key == pygame.K_RETURN:
            ...
            if (
                (event.key >= pygame.K_0 and event.key <= pygame.K_9)
...
class EditTagDialog(Dialog):
    ...
    def evalResult(self):
        ...
        color = (
            float(self.inputBoxes[1].getText()),
            float(self.inputBoxes[2].getText()),
            float(self.inputBoxes[3].getText()),
        )
        colorVar = (
