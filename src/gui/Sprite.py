...
class TextDisplayer(Sprite):
    ...
    def draw(self):
        ...
        if self.lastText != text or self.lastColor != color:
            ...
            if self.listID != -1:
                glDeleteLists(self.listID, 1)
            (self.texture, self.listID, self.renderedSize) = GLUtil.drawText(
                self.getFont(),
                color,
                text,
                self.getPosn(),
...
class UnitBarDisplayer(Sprite):
    ...
    def draw(self, width, posn):
        rightR = self._rightColor[0] * width - self._leftColor[0] * (1.0 - width)
        rightG = self._rightColor[1] * width - self._leftColor[1] * (1.0 - width)
        rightB = self._rightColor[2] * width - self._leftColor[2] * (1.0 - width)
        GLUtil.drawBar(
            self._leftColor, [rightR, rightG, rightB, 1.0], self._width * width, self._height, posn
        )
...
class TextMenu(Sprite):
    def __init__(self, posn, width, nOptions):
        ...
        self.box = TextDisplayerBox(self.displayers, posn, width, nOptions)
        self.selectedOption = 0
        self._showing = False

        (textureID, image, renderedSize) = GLUtil.renderTextToTexture(
            Resources.font(size=16, bold=True), (255, 255, 255, 255), ">", False, None
        )
    ...
    def draw(self):
        if self._showing and len(self.options) > 0:
            ...
            GLUtil.drawAt((cursorX, cursorY), self.cursorTexture, self.cursorImage)
...
class SpecialMenu(TextMenu):
    ...
    def update(self, time):
        # Imported lazily: ScenarioGUI -> Cursor -> Sprite would otherwise
        ...
        if self.selectedUnit != None:
            ...
            for i in range(0, len(self.options)):
                ...
                correctWeapon = self._abilities[i].correctWeapon(self.selectedUnit.weapon())
            ...
            if self._showing and enabled:
                ...
                ScenarioGUI.get().setTopText(ability.description() + _(" (%d SP)") % ability.cost())
...
class DamageDisplayer(Animation):
    def __init__(self, damageAmount, beneficial=NEUTRAL, delay=0.0):
        if beneficial == BENEFICIAL:
            ...
            color = WHITE
        self._time = -delay
        (self._texture, i, isize) = GLUtil.renderTextToTexture(
            Resources.font(size=48), color, str(damageAmount), False, None
        )
    ...
    def draw(self):
        ...
        if self._time < 0.0:
            ...
            alpha = 1.0
        GLUtil.drawFloatingText(self._texture, self._aspectRatio, height, alpha)
    ...
    def __init__(self, unit, delay=0.0, attackStyle="melee"):
        ...
        if wimage != None:
            ...
            for image in oimages:
                self._otexture.append(GLUtil.makeTexture(Resources.image(image))[0])
