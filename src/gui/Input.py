...
def postUserEvent(type_, data={}):
    e = Event(type_, data)
    pygame.event.post(pygame.event.Event(pygame.USEREVENT, {"event": e}))
...
class Input(object):
    def __init__(self, joystickID):
        ...
        if pygame.joystick.get_count() > joystickID:
            ...
            self._joystick.init()
            logger.debug(
                ("Initialized joystick %d " + "(buttons: %d, hats: %d, axes: %d)")
                % (
    ...
    def joyButton(self, number):
        if self._joystick is None or self._joystick.get_numbuttons() <= number:
            return False
        return self._joystick.get_button(number)

    def joyHat(self, number, axis):
        if self._joystick is None or self._joystick.get_numhats() <= number:
            return 0
        hat = self._joystick.get_hat(number)
        return hat[axis]

    def joyAxis(self, number):
        if self._joystick is None or self._joystick.get_numaxes() <= number:
            return 0.0
    ...
    def updateGuiEvents(self, timeElapsed):
        ...
        self.buttonPressed(timeElapsed, START_CHAT, keysPressed[pygame.K_t])
        self.buttonPressed(timeElapsed, FPS, keysPressed[pygame.K_f])
        self.buttonPressed(timeElapsed, TOGGLE_SOUND, keysPressed[pygame.K_s])
        self.buttonPressed(timeElapsed, TOGGLE_FULLSCREEN, keysPressed[pygame.K_F12])
        self.buttonPressed(
            timeElapsed,
            CURSOR_UP,
            keysPressed[pygame.K_UP] or self.joyAxis(5) < -0.8 or self.joyHat(0, 1) == 1,
        )
        self.buttonPressed(
            timeElapsed,
            CURSOR_DOWN,
            keysPressed[pygame.K_DOWN] or self.joyAxis(5) > 0.8 or self.joyHat(0, 1) == -1,
        )
        self.buttonPressed(
            timeElapsed,
            CURSOR_LEFT,
            keysPressed[pygame.K_LEFT] or self.joyAxis(4) < -0.8 or self.joyHat(0, 0) == -1,
        )
        self.buttonPressed(
            timeElapsed,
            CURSOR_RIGHT,
            keysPressed[pygame.K_RIGHT] or self.joyAxis(4) > 0.8 or self.joyHat(0, 0) == 1,
        )
        self.buttonPressed(
            timeElapsed, CURSOR_ACCEPT, keysPressed[pygame.K_RETURN] or self.joyButton(1)
        )
        self.buttonPressed(
            timeElapsed, CURSOR_CANCEL, keysPressed[pygame.K_ESCAPE] or self.joyButton(2)
        )
        self.buttonPressed(
            timeElapsed, LOWER_CAMERA, keysPressed[pygame.K_PAGEUP] or self.joyButton(6)
        )
        self.buttonPressed(
            timeElapsed, RAISE_CAMERA, keysPressed[pygame.K_PAGEDOWN] or self.joyButton(7)
        )
        self.buttonPressed(
            timeElapsed,
            ROTATE_CAMERA_CCW,
            keysPressed[pygame.K_LEFTBRACKET] or keysPressed[pygame.K_HOME] or self.joyButton(4),
        )
        self.buttonPressed(
            timeElapsed,
            ROTATE_CAMERA_CW,
            keysPressed[pygame.K_RIGHTBRACKET] or keysPressed[pygame.K_END] or self.joyButton(5),
        )
        self.buttonPressed(timeElapsed, RAISE_TILE, keysPressed[pygame.K_EQUALS])
        self.buttonPressed(timeElapsed, LOWER_TILE, keysPressed[pygame.K_MINUS])
        self.buttonPressed(
            timeElapsed,
            RAISE_CENTER,
            (
                keysPressed[pygame.K_s]
                and not (keysPressed[pygame.K_LSHIFT] or keysPressed[pygame.K_RSHIFT])
            ),
        )
        self.buttonPressed(
            timeElapsed,
            ...
            (
                keysPressed[pygame.K_s]
                and (keysPressed[pygame.K_LSHIFT] or keysPressed[pygame.K_RSHIFT])
            ),
        )
        self.buttonPressed(
            timeElapsed,
            RAISE_B_BL_CORNER,
            keysPressed[pygame.K_w]
            and not (keysPressed[pygame.K_LSHIFT] or keysPressed[pygame.K_RSHIFT]),
        )
        self.buttonPressed(
            timeElapsed,
            RAISE_L_FL_CORNER,
            keysPressed[pygame.K_a]
            and not (keysPressed[pygame.K_LSHIFT] or keysPressed[pygame.K_RSHIFT]),
        )
        self.buttonPressed(
            timeElapsed,
            RAISE_F_FR_CORNER,
            keysPressed[pygame.K_x]
            and not (keysPressed[pygame.K_LSHIFT] or keysPressed[pygame.K_RSHIFT]),
        )
        self.buttonPressed(
            timeElapsed,
        ...
        self.buttonPressed(timeElapsed, UNDO, keysPressed[pygame.K_BACKSPACE])
        ...
        if mousePressed[2] and mouseMotion[0] != 0:
            postUserEvent(ROTATE_CAMERA, {"amount": mouseMotion[0]})
        if abs(self.joyAxis(0)) > 0.8:
            amount = self.joyAxis(0) * timeElapsed * 180.0
            postUserEvent(ROTATE_CAMERA, {"amount": amount})

        # Pitch camera
        if mousePressed[2]:
            postUserEvent(PITCH_CAMERA, {"amount": mouseMotion[1] / 3.0})
        if abs(self.joyAxis(1)) > 0.8:
            amount = self.joyAxis(1) * timeElapsed * 90.0
            postUserEvent(PITCH_CAMERA, {"amount": amount})

        # Translate view
        if mousePressed[0]:
            x = mouseMotion[0] / 750.0
            y = mouseMotion[1] / 750.0
            postUserEvent(TRANSLATE_CAMERA, {"amount": (x, y)})
