...
class FontLoader(object):
    def __init__(self):
        self.fonts = {}
        self.files = {
            "sans": {False: "vera/Vera.ttf", True: "vera/VeraBd.ttf"},
            "serif": {False: "vera/VeraSe.ttf", True: "vera/VeraSeBd.ttf"},
            "mono": {False: "vera/VeraMono.ttf", True: "vera/VeraMoBd.ttf"},
        }
...
class TextureLoader(object):
    ...
    def __call__(self, textureName):
        if textureName not in self.cache:
            ...
            i = pygame.transform.scale(i, (self._textureSize, self._textureSize))
...
class AbilityLoader(object):
    ...
    def __call__(self, abilityName):
        if abilityName not in self.cache:
            ...
            module = compile("from engine.Equipment import Weapon", "Equipment.py", "exec")
            eval(module, globalVars)

            module = compile("from engine.Range import *", "Range.py", "exec")
            eval(module, globalVars)
            module = compile(
                "from engine.Ability import ACTION, FRIENDLY, HOSTILE, "
                + "FRIENDLY_AND_HOSTILE, WEAPON_SOUND",
                "Ability.py",
                "exec",
            )
            eval(module, globalVars)
            module = compile("from engine.Effect import *", "Effect.py", "exec")
            eval(module, globalVars)
            ...
            ability = Ability.Ability(
                name, description, cost, targetType, requiredWeapons, range_, aoe, effects, sound
            )
...
class ClassLoader(object):
    ...
    def _loadClass(self, className):
        ...
        self.cache[className] = Class_.Class(
            classData["NAME"],
            abilities,
            spriteRoot,
            classData["MOVE"],
...
class UnitLoader(object):
    def __call__(self, unitName):
        ...
        module = compile(
            "from engine.Unit import MALE, FEMALE, NEUTER, FEMALE_OR_MALE", "Unit.py", "exec"
        )
        eval(module, globalVars)
    ...
    def __call__(self, type_, equipmentName):
        if equipmentName not in self.cache:
            ...
            if type_ == Equipment.WEAPON:
                ...
                self.cache[equipmentName] = Equipment.Weapon(name, stats, weaponType)
            elif type_ == Equipment.ARMOR:
