...
ENDING_CONDITIONS = [Battle.PLAYER_DEFEATED, Battle.DEFEAT_ALL_ENEMIES]

player = Faction(
    id=0, units=[("fighter1", (9, 6)), ("rogue1", (10, 8)), ("healer1", (6, 6)), ("mage1", (11, 7))]
)
...
LIGHTING.addLight(Light.White(brightness=0.3, position=[20, 30, 200]))
...
# results are!
LIGHTING.addLight(
    Light.Point(position=[8, 7, 12], diffuse=[0.5, 0.2, 0.0], attenuation=[0.05, 0.05, 0.01])
)

# Set some fog. This will make the map and units appear to fade away
# into the distance.
LIGHTING.setFog(color=[0.0, 0.0, 0.0], start=10.0, end=30.0)
