...
from engine import Equipment as Equipment
...
class Ability(pb.Copyable, pb.RemoteCopy):
    ...
    def __init__(
        self, name, description, cost, targetType, requiredWeapons, range, aoe, effects, sound
    ):
        ...
        for x, y in affectedSquares:
            ...
            if t != None:
                if self._targetType == FRIENDLY_AND_HOSTILE:
                    ...
                    sourceUnit.faction(), t.faction()
                ):
                    result.append(t)
                elif self._targetType == HOSTILE and Faction.hostile(
                    sourceUnit.faction(), t.faction()
                ):
                    result.append(t)
        return result
