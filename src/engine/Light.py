...
class Environment(pb.Copyable, pb.RemoteCopy):
    ...
    def setFog(self, color=[1.0, 1.0, 1.0], density=1.0, start=1.0, end=100.0):
...
def randomEnvironment(width=0.0, height=0.0):
    ...
    (r, g, b) = (random.uniform(0.2, 0.8), random.uniform(0.2, 0.8), random.uniform(0.2, 0.8))
    lightx = random.uniform(0, width)
    lighty = random.uniform(0, height)
    lighting.addLight(Point(diffuse=[r, g, b], ambient=[r, g, b], position=[lightx, lighty, 500]))
    fogstart = random.uniform(15.0, 25.0)
    fogend = random.uniform(fogstart + 10.0, fogstart + 20.0)
    lighting.setFog(color=((1.0 + r) / 2, (1.0 + g) / 2, (1.0 + b) / 2), start=fogstart, end=fogend)
    return lighting
